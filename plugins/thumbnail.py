import logging
  ...
  async def handle_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
      ...
      if state == "waiting_for_image":
          ...
          if smallest.file_size > 200 * 1024 or smallest.width > 320 or smallest.height > 320:
              await update.message.reply_text("❌ Send a smaller image (under 200KB, 320×320px).")
              return
          ...
          try:
              ...
              await context.bot.send_video(
                  chat_id=update.message.chat_id, video=user["video_file_id"],
                  thumbnail=largest.file_id, caption=user["video_caption"],
                  caption_entities=entities, supports_streaming=True,
  ...
  async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
      ...
      if saved:
          try:
              await context.bot.send_video(
                  chat_id=update.message.chat_id, video=video.file_id,
                  thumbnail=saved, caption=update.message.caption,
      ...
      if query.data == "check_join":
          if await is_user_joined(query.from_user.id, context):
              await query.edit_message_text(
                  ...
                  "⚡ <b>Powered by:</b> @World_Fastest_Bots", parse_mode="HTML"
              )
          else:
